...
class Express
{
    ...
    public function __construct(Options $options)
    {
        ...
        $this->folder  = new Folder($options);
     ...
     * archive thread
     *
     * @param string $threaId
     * @return array
     */
    public function archive(string $threaId)
    {
        return Helper::isLabel($this->options) ?
        $this->updateLabels($threaId, null, ['inbox']) :
        $this->updateFolder($threaId, 'archive');
    }
    ...
    {
        $folderId   = null;
        $allFolders = $this->folder->getFoldersList();
    ...
    private function updateLabels(string $threadId, $add = [], $del = [])
    {
        ...
        $tmpLabels = [];
        $allLabels = $this->label->getLabelsList();
        $nowLabels = (new Thread($this->options))->getThread($threadId)[$threadId]['labels'];
